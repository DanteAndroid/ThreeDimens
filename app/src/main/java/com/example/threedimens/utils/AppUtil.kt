...
object AppUtil {
    ...
    fun goMarket(activity: AppCompatActivity) {
        try {
            ...
            val intent =
                Intent(Intent.ACTION_VIEW, Uri.parse("http://www.coolapk.com/apk/" + BuildConfig.APPLICATION_ID))
            activity.startActivity(intent)
        }
    ...
    fun donate(activity: AppCompatActivity) {
        ...
        if (AlipayZeroSdk.hasInstalledAlipayClient(activity.applicationContext)) {
            if (deviceId.isEmpty()) {
                ...
                Handler().postDelayed({ AlipayZeroSdk.startAlipayClient(activity, "fkx06410coelwa7winexd90") }, 500)
            }
        } else {
