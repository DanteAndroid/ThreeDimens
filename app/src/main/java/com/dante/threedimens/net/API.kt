package com.dante.threedimens.net

/**
 * Net request APIs and CATEs
 */
object API {

    const val GITHUB_RAW = "https://raw.githubusercontent.com/DanteAndroid/ThreeDimens/master/"
    const val DOWNLOAD_BASE = "https://github.com/DanteAndroid/ThreeDimens/releases/download/"

    //豆瓣
    const val CATE_DB_BREAST = "2"
    const val CATE_DB_BUTT = "6"
    const val CATE_DB_SILK = "7"
    const val CATE_DB_LEG = "3"
    const val CATE_DB_RANK = "5"

    //妹子图
    const val CATE_MZ_HOT = "hot"
    const val CATE_MZ_SEXY = "xinggan"
    const val CATE_MZ_INNOCENT = "mm"
    //美图录
    const val CATE_MTL_CHINESE = "guochan"
    const val CATE_MTL_MODEL = "t/nenmo"
    const val CATE_MTL_BREAST = "t/meixiong"
    const val CATE_MTL_COS = "t/cosplay"

    //wall haven
    const val CATE_WH_GIRL = "girl"
    const val CATE_WH_RANDOM = "random"
    const val CATE_WH_ANIME = "anime"
    const val CATE_WH_FANTASY = "fantasy"
    const val CATE_WH_LANDSCAPE = "landscape"
    const val CATE_WH_DARK = "dark"
    const val CATE_WH_SIMPLE = "simple"

    // Dan booru
    const val CATE_DAN_HOT = "order:rank"

    //Base API
    const val GANK_BASE = "http://gank.io/api/"
    const val DB_BASE = "https://www.buxiuse.com/"
    const val MZ_BASE = "https://www.mzitu.com/"
    const val WALL_BASE = "https://wallhaven.cc/"
    const val YANDE_BASE = "https://yande.re/"
    const val SAFEBOORU_BASE = "https://safebooru.org/"
    const val DANBOORU_BASE = "https://danbooru.donmai.us/"
    const val `3DBOORU_BASE` = "http://behoimi.org"
    const val MEITULU_BASE = "https://www.meitulu.com/"

    //H API
    const val YAKEXI_BASE = "http://bww.yakexi.biz/pw/"
    const val H_MAIN = YAKEXI_BASE + "thread.php?fid="
    const val SHT_BASE = ""


}
